...
def preflight(
    ...
    **返回值在生产路径上目前没有消费者**（`run_pipeline` 丢掉它，理由见
    `pipeline.run_timeline` 的 docstring：复用它要么省不到东西、要么静默关掉帧对齐）。
    末尾那次 `probe_duration` 仍然必须跑 —— 它本身就是一项检查：时长 <= 0 会抛错，
    挡住截断/0 字节的容器（见 probe_duration 的 docstring）。返回值只是顺手交出来的。
