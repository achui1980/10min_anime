...
黄金样本上的预期输出（前 5 条，**示意**——只有 `gap:` 那一路是实测数字，
`density:` 与 `llm:` 两路的强度要等 Task 11/12 实测后回填）：

```
强度  时间戳               触发信号                             内容
5    00:22:02-00:22:28   density:0.92 + gap:19.8s + llm:修罗场   "我好想你" 哭扑
5    00:20:58-00:21:15   llm:黄色梗 + gap:3.7s                  "硬不起來的藥"
3    00:18:18-00:18:26   llm:福利（该段无间隙，纯语义信号）        "一起泡澡吧"
...
`signals`
- 检出 26 个 ≥ 3 秒的无字幕间隙。这是**候选池**，不是高光集合；断言写成召回式
  （6 个已知视觉高光各自被某个间隙覆盖，重叠 ≥ 1 秒）+ 一个宽松总数区间 `20 <= n <= 32`
- 最长间隙 19.813 秒（00:22:08.367 → 00:22:28.180），起止误差 < 10ms
- 第 269 行的 `char_rate` 是全集最小值
- gap 通道给出强度 4 的只有两个 ≥ 15 秒的间隙：00:22:08.367 → 00:22:28.180（定格收尾）
  与 00:02:14.633 → 00:02:33.486（学院全景）。其余高能点的强度依赖 density 与 LLM 两路，
  在 `signals/density.py` 与 `signals/aggregate.py` 实测之前不写死具体时间点
