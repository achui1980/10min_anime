...
├── tests/
│   ├── fixtures/
│   │   └── saijo_e02.srt       黄金样本字幕：《才女的侍從》第2集，406 行
│   └── ...
...
真实字幕的脏数据密度很高。以黄金样本《才女的侍從》第 2 集（406 行）为例，实测出现的问题类型：
...
| 401, 403, 404, 406 | `HE IYA Synergy SP J.C.STAFF…` / `作詞 前島亞美 作曲·編曲…` | ED staff |
...
10. **跨行同句合并** — 上一行末尾无终止标点、与下一行间隔 < 0.3 秒、且 speaker 相同，合并为一条
...
- `ed_range = [1348.180, 1416.622]` — 第 401 行 `HE IYA Synergy SP J.C.STAFF…` 起至第 406 行结束

`project.yaml` 的 `op_range` / `ed_range` 若非空则直接覆盖自动检测结果。

### 6.3 signals — 高能点检测

这是本设计中最重要的发现。在黄金样本上统计所有 ≥ 3 秒的无字幕间隙：

| 间隙区间 | 时长 | 实际内容 |
|---|---|---|
| 00:02:14 → 00:02:33 | 18.9s | 贵皇学院全景仰拍演出 |
| 00:22:08 → 00:22:28 | 19.8s | 修罗场后的定格拉远收尾 |
| 00:20:35 → 00:20:47 | 11.6s | 女仆静音无声凝视的长镜头 |
| 00:12:17 → 00:12:25 | 8.3s | 天王寺美丽登场亮相演出 |
| 00:21:57 → 00:22:03 | 5.5s | 被认出后的震惊定格 |
| 00:18:20 → 00:18:25 | 4.7s | 泳装登场定格 |
| 00:21:09 → 00:21:13 | 3.7s | 递药丸后的反应镜头 |

**本集全部视觉高光，无一例外，全部落在无字幕间隙里。** 这不是巧合——动画的演出语法就是「该炫的时候不说话」。

推论：「长无字幕间隙 = 演出高光」这条启发式规则，计算成本是零（纯时间戳算术），在黄金样本上召回率 100%。它比音频能量曲线更便宜，也比 CLIP 视觉索引更准。
...
黄金样本《才女的侍從》第 2 集存为 `tests/fixtures/saijo_e02.srt`，406 行，时长 00:00:07–00:23:36。

**确定性阶段做严格断言：**

`ingest`
- 第 51、52、53、54、401、403、404、406 行 `kind == "credits"`
- 第 375 行 `kind == "noise"`
