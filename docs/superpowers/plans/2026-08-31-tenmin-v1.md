...
`tests/fixtures/saijo_e02.srt` 必须是《才女的侍從》第 2 集的**真实原始 SRT 文件**（繁体中文，406 个字幕块，首块 `00:00:07`，末块 `00:23:36`）。后续 Task 5–11 的全部断言都是从这个具体文件上量出来的行号与时间戳，用别的文件或手工伪造的片段会全部失败。
...
assert len(blocks) == 406, f"期望 406 个字幕块，实际 {len(blocks)}"
print("OK")
PY
```
Expected: `blocks: 406` 然后 `OK`

如果这一步报 STOP 或块数不符，**停下来告诉用户**：「需要你把《才女的侍從》第 2 集的原始 SRT 文件放到 `tests/fixtures/saijo_e02.srt`（406 块，UTF-8）。这是全部回归测试的基准数据，我不能替你生成。」不要继续后面的任务。
...
def test_decode_bytes_gbk():
    assert "你好" in decode_bytes("你好世界".encode("gbk"))
...
2. **聚簇 `max_gap = 30.0` 秒**（spec 写 8 秒，见前文偏离声明 6）。op 候选条件：簇起点落在 `[60, 300]`、跨度落在 `[40, 120]`，多个候选取跨度最大的。ed 候选条件：簇起点 `>= duration - 120`，取起点最早的。
...
def test_golden_sample_credit_lines(golden_track):
    ...
    for idx in (51, 52, 53, 54, 401, 403, 404, 406):
...
Expected: PASS，32 passed（test_normalize.py 19 个 + test_credits.py 13 个，含 Task 8 里之前被跳过的两个 golden 测试）
...
for ln in t.lines:
    if ln.idx in (41,51,52,53,54,56,57,67,105,187,200,201,202,291,375,401,403,404,406):
...
单集模式一次 LLM 调用就够：406 行字幕约 6000 字符，加信号清单和 few-shot 范例，总量不到 3 万 token，无需分片。
...
- [ ] `tests/fixtures/saijo_e02.srt` 已就位且是 406 块（Task 1 门禁）
