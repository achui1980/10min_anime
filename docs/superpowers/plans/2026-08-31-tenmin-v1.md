...
> ⚠️ **泛化复验后的修订（已并入下方代码块，代码块与磁盘逐字节一致）**
>
> - **OP 识别命中率 1/5 → 5/5**，在 4 个第三方泛化样本 + 1 个黄金样本上实测。
> - **两类番剧的 OP 形态是相反的，两条识别路径必须并存**：有 staff 字幕的 OP 靠 `credits` 聚簇识别（静区法看不到它，因为整段被 staff 字幕填满）；没有 staff 字幕的 OP 一条字幕都没有，整段就是一个 90 秒级静默区，只能靠 `_op_from_silence` 兜底（聚簇法必挂，因为没有 credits 行可聚）。**不要以「简化」为名删掉任何一条**。
> - **`_op_from_silence` 里「先按 `[OP_MIN_SILENT_SPAN, OP_MAX_SILENT_SPAN]` = `[60,120]` 过滤、再取最长」的顺序不能反。** 若改成「先取窗内最长、再验时长」：某集片头窗内只要存在一个超过 120 秒的非-OP 静区（例如整段无对白的长过场），它就会挤掉真正的 90 秒 OP 候选，使兜底直接返回 `None`——本该救回来的 OP 反而丢了。这个顺序在现有 5 个样本上是纯 no-op（没有一集在 OP 窗内存在 >120s 的干扰静区），所以真实数据验证不到它，由 `test_op_from_silence_prefers_gap_inside_span_window` / `test_op_from_silence_ignores_gap_outside_span_window` 两个合成数据单测锁死。
> - **`_KEYWORDS` 拆成 `_KEYWORDS_ALWAYS` / `_KEYWORDS_IN_WINDOW` 两组的原因**：原来 23 词无条件子串匹配，`演出` 命中了真台词「你现在的说话方式也是演出来的吧」，该行被踢出对白轨，两侧间隙被虚假合并成一个 4.296 秒的假高光直接喂给 LLM。拆分标准是「是否同时是通用中文词」：日文专有写法与高度特定复合词（`製作委員会` `作詞` `フォント` `主題歌` 等 13 条）无条件生效；中文歧义词（`制作` `协力` `作画` `监督` `脚本` `演出` `原作` 等 10 条）只在片头片尾窗内才敢认。规则编号：1（版权标记）、2a（ALWAYS 关键词）、3（书名号包裹且与剧名重合）、6（标题卡）在 `if not in_credit_window: return False` 之前无条件生效；2b（IN_WINDOW 关键词）、4（纯人名罗列）、5（拉丁字母占比）在那一行之后，仅窗内生效。
> - **两个已知局限，刻意不修，避免过拟合到 5 个样本**：
>   1. ED 起点锚在「第一条 ED staff 字幕」，而 ED 曲子通常比 staff 字幕早起 20–50 秒，所以推断出的 ED 区间系统性偏晚。把它前移会打挂黄金样本里 1328.367–1348.180 那个 19.8 秒「定格收尾」★ 高光（会被并进 ED 而丢掉）。
>   2. 规则 4 的双段人名正则会把 `美丽 辛苦妳了`（2 段、6 个 CJK）误判成 staff 行，而真 staff 名单 `河原正信 有賀史英` 也是 2 段（8 个 CJK）。靠字数门槛区分属于过拟合，靠语义区分需要 NLP，两者都不做。
...
Expected: PASS，14 passed，3 deselected
全库预期：`uv run pytest -q` → 344 passed / 0 failed / 1 skipped。
曾经 failed 的 `test_gap_count_is_in_usable_range[akujo_e01]`（34 > 30）与 `[saijo_e05]`（33 > 30）
已在 commit `fe4b78c` 修掉：该测试改为只统计 `strength >= 3` 的可用间隙。
唯一剩下的 1 个 skipped 是 `test_real_llm_snapshot`，它需要真实 `TENMIN_GEMINI_API_KEY` 才会执行。
...
处理一个 cue 的固定顺序：
`clean_text`（文本层）→ `split_dual_track` → 逐段 `extract_prefix` → 在折叠换行**之前**算 `suspect` → 折叠换行 → 判 `kind`（noise → credits → screen_text → monologue/dialogue）。全部 cue 处理完后推断 OP/ED。

跨行合并的条件刻意保守（两行都不 suspect、必须来自不同 cue、合并后 ≤40 字），因为合并会让第二行的 SRT 序号从 `idx` 里消失——它会被记进 `merged_from`，测试靠 `lines_for_idx` 仍能定位。

**⚠️ 跨 cue 合并在管线里默认关闭（`build_track(..., merge_lines=False)`），这是在黄金样本上实测后的设计修正，不要改回默认打开。**三条理由：

1. **主闸门空转**：spec 规则 ⑩ 的三个判据里 `prev.speaker == nxt.speaker` 本该是主闸门，但这份字幕绝大多数行没有 `(角色名)` 前缀，条件退化成 `None == None` 恒为真。
2. **两个副闸门被数据证伪**：黄金样本 405 条 cue 的相邻间隔中位数是 **0.001 秒**（229/404 个间隔 < 0.3s），`MERGE_MAX_GAP` 筛不掉任何东西；全集只有 **9/405** 行以终止标点结尾（末字直方图 了35 嗎34 吧22 的14），`TERMINAL_PUNCT` 同样筛不掉任何东西。照原设计跑会触发 126 次合并（408 行 → 282 行），并且**语义错误**——实测把 `是` + `您有什么吩咐` + `你的讲话方式什么` 三个不同说话人黏成了一条。
3. **这份字幕的句内折行本来就在 cue 内部用 `\n` 表示**（第 200/201/202 行就是这个形状），已由 `_fold` 处理；跨 cue 相邻几乎总是换话轮。

`merge_continuations` 仍作为带开关的纯函数保留并单测覆盖——带说话人标注的字幕源（v3 Faster-Whisper 输出，或带 `(角色名)` 前缀的字幕）可以显式传 `merge_lines=True` 用上它。
...
Expected: PASS，42 passed（test_normalize.py **27** 个 + test_credits.py 15 个，含 Task 8 里之前被跳过的两个 golden 测试）
再跑全库 `uv run pytest -q`，预期 **184 passed / 0 skipped**

（`test_normalize.py` 里三处 `@pytest.mark.parametrize` 会展开：suspect 5 个、dual_track 2 个、title_card 2 个，所以收集数是 27 而不是函数个数。）
...
这是 v1 最重要的一条规则。黄金样本上的实证结论：**本集视觉高光 6/7 落在长无字幕间隙里**（动画的演出语法就是「该炫的时候不说话」）。零成本，不需要看画面。唯一的例外是 `00:18:20→00:18:25`（泳装定格），实测该区段是连续对白（334 `好宽敞` / 335 `伊月` / 336 `一起泡澡吧`），字幕轨里没有间隙。

> ⚠️ **`find_silent_gaps` 是候选检测器，不是高光选择器。** 黄金样本上实测检出 **26** 个 ≥3s 间隙，不是 7 个。spec §6.1 那 7 个区间是人工看片挑的视觉高光，它们散落在时长排名 **1 / 2 / 3 / 7 / 11 / 18**（第 7 个不在间隙里），中间夹着 4 个更长的**非**高光间隙（10.2s / 9.8s / 8.9s / 8.1s）——**任何 `MIN_GAP_SECONDS` 阈值都还原不出那 7 个**，因为它们在时长上不可分离。
>
> 所以本任务的黄金断言是 **recall**（每个已知视觉高光都被某个检出间隙覆盖）而不是 **precision**（数量相等）。把 26 个候选收敛到 ~7 个 highlight 是 Task 12 `aggregate.py` 的职责（它会叠加 density 信号并做 2s 邻近聚簇）。**不要为了让数字变成 7 而抬阈值。**
...
Run: `uv run pytest tests/test_gaps.py -v`
Expected: PASS，27 passed（16 单元 + 11 黄金：`test_golden_gaps_cover_known_visual_peaks` 是 6 参数化）
全库：`uv run pytest -q` → 211 passed。

如果 `test_golden_gaps_cover_known_visual_peaks` 某个参数红了，说明该视觉高光在字幕轨里没有对应的 ≥3s 间隙，即核心假设在这一处不成立——先手工核对那个时间段的对白密度（很可能像 00:18:20 泳装定格那样其实是连着说话的），确认后从 `KNOWN_VISUAL_PEAKS` 里移除并同步 spec §6.1，**不要**下调 `MIN_GAP_SECONDS` 去凑。

如果 `test_golden_gap_count_is_a_candidate_pool` 越出 `[20, 32]`，失败信息会打出实际检出的全部区间。**注意：数量偏多不是「台词被误判成 credits/noise 造成的假间隙」。** 黄金样本实测过：26 个间隙里只有一个含非语音行（idx 375 的孤立破折号 `-`，`kind="noise"`），而它的效果是把 `[1233.606, 1246.995]` 这个真间隙**拉长**（跳过它 → 13.389s；把它当语音边界 → 11.596s），不是凭空造出间隙。spec §6.1 原先记的 `00:20:35→00:20:47 (11.6s)` 就是当年把这个 `-` 当成语音边界量出来的，正确跳过非语音行后应为 **13.389s**。所以数量异常应该先怀疑 OP/ED 区间（Task 8）算错了，而不是 kind 分类。
...
- **低字密度**：`char_rate = 非空白字符数 / 时长`，`< 0.4 × 全集中位数` 且 `时长 >= 2.0s` → 强度 3。实证依据：黄金样本第 269 行「才没有染呢我」6 字 / 6.589s = 0.91 字/秒，是本集低语速的标杆之一（对应天王寺被问染发炸毛的尖叫镜头）。它不是全集最慢——第 237 行「这」1 字拖 1.876 秒只有 0.53 字/秒，但被 `LOW_DENSITY_MIN_SECONDS = 2.0` 过滤掉；在 duration ≥ 2.0 的子集里第 16 行「什么」（0.89 字/秒）也比它慢。低字密度 = 情绪爆发。

> **黄金样本实测基线（Task 12 聚合要用）**：`median_char_rate = 3.597` 字/秒，低密度阈值 = `0.4 × 3.597 = 1.439`。
> 全集 spoken 行（dialogue + monologue，text 非空，duration > 0）共 **399** 条。
> `find_low_density` 检出 **9** 条（全部 strength 3，anchor 分别是 16 / 77 / 201 / 269 / 288 / 331 / 399 / 400 / 404）。
> `find_density_shifts` 检出 **6** 条（全部 strength 2，落在 120–150 / 240–270 / 480–510 / 510–540 / 810–840 / 990–1020 秒）。
> 加上 Task 10 的 26 条 gap，signals 层交给 `aggregate.py` 的总候选是 **41** 条。
>
> ⚠️ 两条已知伪影，Task 12 聚合时要考虑按 `op_range` / `ed_range` 剔除：`120–150` 秒那条紧邻 OP 起点 153.486 之前，`240–270` 秒那条（z=+2.67，最强）正好是 OP 结束后对白恢复；另 anchor 404 的 low_density 信号落在 ED 区间内（`矢田晶子`，Task 9 已知的假台词）。
...
全库 `uv run pytest -q` Expected: 228 passed / 0 skipped
...
> ⚠️ **下游影响（Task 10 实测修正，同时适用于 Task 13）：** `find_silent_gaps` 在黄金样本上产出 **26** 条 `source="gap"` 信号，强度分布 `{4: 2, 3: 6, 2: 18}`，**不是**计划早期各处按「7 个间隙」写的数字。本任务及 Task 13 里凡是按 7 条 gap 信号推算出来的下游断言（簇数量、Highlight 条数、few-shot 资产里的信号清单）都要按这组实测值重算。把 26 个候选收敛到 ~7 个视觉高光正是本任务的职责——收敛发生在这里，不在 `gaps.py`。

> 📏 **Task 12 实测基线与两处设计修正（实现完成后回填）：**
>
> - **黄金样本实测**：41 条候选信号（26 `gap` + 9 `low_density` + 6 `density_shift`）→ **33** 个 highlight，强度分布 `{2: 16, 3: 13, 4: 2, 5: 2}`，`median_char_rate = 3.597`，`silent_gaps = 26` 条（**不是**早期文档里的 7 条）。
> - ⚠️ **`density_shift` 只贡献强度加成与 trigger 标记，不参与 highlight 边界计算，也不能桥接两个簇。** 它的区间是 `density.py` 里死板的 30 秒统计桶（`480–510`、`510–540` 这类首尾相接的网格），语义是「这一片区域的叙事节奏换挡了」，不是精确的演出区间。按老实现让它平等参与 `_cluster` 的链式邻接判定，实测造出两个坏结果：(1) 一个假的 62.1 秒「无台词演出段」`480.000–542.083`，triggers `['gap:3.0s', 'gap:8.9s', 'gap:9.8s', 'shift:z=+1.63', 'shift:z=-1.72']`——两条 30s 桶把三条本来相隔几十秒的真间隙串成一条，片子里并不存在这个段落；(2) highlight 起点被网格值污染，`120.000–153.486` 的真实无字幕间隙起点是 `134.633`，差了 14.6 秒（同类还有 `990.000–1020.000`，真实起点 `1012.760`）。这些时间码会直接进「解说方案对照表」当剪辑入点，精度不能让。修法：邻接链只由非 `density_shift` 信号驱动；边界只用簇内的非 shift 信号算 min/max；整簇只有 `density_shift` 时（黄金样本里 `240–270`、`810–840` 两条）才照原样保留桶区间，这类「纯节奏变化」提示是对的。`_summary` 也改成只看边界子集，否则 duration 会报 33.5s 而 start/end 只跨 18.9s。
> - ⚠️ **`KNOWN_VISUAL_PEAKS` 的断言是召回式的**（每个高光被某个 highlight 覆盖，重叠 ≥1s），**不断强度**。老版本要求覆盖 `1330.0 / 140.0 / 1240.0` 的 highlight 强度 `>= 4`，实测是 5 / 5 / **3**，第三个必红。根因是把 recall 和 precision 混成一个断言：6 个人工高光对应的无字幕间隙时长是 18.9 / 8.3 / 13.4 / 3.7 / 5.5 / 19.8 秒，横跨 3.7–19.8 秒，必然被 `gaps.py` 的三档强度表 `((15.0, 4), (8.0, 3), (0.0, 2))` 打散到 2/3/5 三档——要求它们全部 ≥4 等价于要求「13.4 秒的间隙必须和 19.8 秒的间隙同档」，逻辑上不可能成立。强度那一半单独由 `test_build_report_golden_long_gaps_reach_top_strength` 守（两个 ≥15s 间隙顶到强度 5）。**把 33 个候选收敛到 ~7 个解说节点、以及节点之间的优先级排序，是 LLM 在 Task 17 `script/single.py` 阶段的职责，不是 `aggregate` 的职责。**
...
Run: `uv run pytest tests/test_aggregate.py -v`
Expected: PASS，21 passed（13 个单元测试 + `build_report` 接线 + 6 个参数化召回用例 + 长间隙顶格强度）
Run: `uv run pytest -q`
Expected: 249 passed / 0 skipped
...
> ⚠️ **两处 fixture 数据陷阱（已在黄金样本上实测修正）**
>
> 1. **clip 时间戳不能落进 OP/ED 区间。** `track` fixture 用的是黄金样本真值 `op_range = (153.486, 224.681)` / `ed_range = (1348.18, 1416.622)`，而 `validate.py` 的 `_fully_inside` 会把完全落在这两个区间里的 clip 当片头/片尾曲静默丢弃。原计划的 `10.0 + i * 100` 让第 3 个 beat 落在 `210.0–215.0`，整段被吞 → beat 零 clip → 抛 `ScriptValidationError` → 重试 → `FakeProvider` 响应用尽 → 6 个测试连锁失败。改成 `i * 300`（起点 10 / 310 / 610）后全部避开。**后续任何任务只要在 track fixture 上凭空造 clip 时间戳，都要先对照这两个区间。**
> 2. **anchor_lines 要么给对，要么别给。** `track` fixture 只有 6 行、覆盖 7–51s。给 310s / 610s 的 clip 硬编码 `anchor_lines=[1]`（起点 7.12s）会让偏差超过 `ANCHOR_TOLERANCE_SECONDS = 5.0`，触发 warning 并以字幕时间覆写 clip 起点，`assert warnings == []` 必挂。现在只有 beat1 带 `anchors=[1]`（偏差 2.88s，在容差内、不产 warning，anchor 逻辑仍有覆盖），beat2/beat3 传空列表。
>
> **实测 LLM prompt 规模（黄金样本）**：`build_dialogue_block` 399 行 / 24,126 字符（dialogue 396 + monologue 3，credits 8 与 noise 1 被 `SPEECH_KINDS` 过滤），`build_highlight_block` 33 行 / 2,141 字符，`build_user_prompt` 总长 **31,254 字符**（其中模板 + few-shot 固定开销 4,987）。粗估 20k–31k input token，「单集一次调用无需分片」成立，但已贴着上界；若 few-shot 再加范例需复核。

黄金样本实测：对白块 399 行 24,126 字符，信号清单 33 行 2,141 字符，加提示词模板与 few-shot 范例的 4,987 字符固定开销，`build_user_prompt` 总长 31,254 字符，粗估 20k–31k input token，单次调用即可，无需分片。
...
再跑全库 `uv run pytest -q`，预期 **280 passed / 0 skipped**
...
> **TODO（Task 10 实测遗留，同样影响 Task 21 的示例数据）：** 下面示例表 Hook 行把 `00:18:20.100 - 00:18:24.800` 标了 ★（即 `is_silent_highlight=True`），但按 Task 16 `validate.py` 的重算规则（clip 与某条无字幕间隙重叠 ≥1.0s 才为 True），这个 ★ 会被算成 **False**——该区段实测是连续对白（334 `好宽敞` / 335 `伊月` / 336 `一起泡澡吧`），字幕轨里没有间隙。执行 Task 18 / Task 21 时需要同步修正示例数据：换成一个真落在间隙里的时间段（例如 `00:20:33.6 - 00:20:47.0`），或者直接去掉这个 ★。**先不要改这两个任务的代码块，等排到它们时一并处理。**
...
> ⚠️ **本任务的测试块里有两处计划自身的错误，已按实测修正：**
> 1. `test_run_signals_writes_signals_json` 原写 `len(reports[0].silent_gaps) == 7`，实测是 **26**。这是 Task 10 遗留的过时数字（`tests/test_aggregate.py` 对同一黄金样本断言的就是 26）。已验证 `run_ingest` 造出的 track 与 `conftest` 的 `golden_track` 逐字节相同，所以两边 `silent_gaps` 必然一致。
> 2. `test_run_pipeline_only_accepts_multiple_stages` 原写 `paths.dialogue(1)` / `paths.signals(1)`，但 fixture 里唯一的 episode 是 `number: 2`，产物必然是 `E02.*`。同一测试块其余 6 处都写的是 `(2)`，只有这两行是笔误。
>
> **实测产物路径**（`Paths(Path("work/saijo"))`）：`01_dialogue/E02.dialogue.json` / `02_signals/E02.signals.json` / `03_script/script.json`（★唯一人工编辑面）/ `out/解说方案.md` / `out/narration.txt`。集数补零来自 `f"E{episode:02d}"`。
>
> **已知取舍**：`run_script` 只用 `tracks[0]` / `reports[0]`。`mode: single_episode` 下若 `episodes` 写了多集，第 2 集及之后会被静默忽略（不报错）。v1 边界内可接受。
...
再跑全库 `uv run pytest -q`，预期 **298 passed / 0 skipped**
...
> **TODO（同 Task 18 头部那条）：** 本任务的示例/夹具数据若沿用了 `00:18:20.100 - 00:18:24.800 ★`，需与 Task 18 一起改掉——该时间段实测不在无字幕间隙里，`is_silent_highlight` 应为 False。另外 `inspect` 打印的 gap 信号在黄金样本上是 **26** 条（强度分布 `{4: 2, 3: 6, 2: 18}`），不是早期计划里写的 7 条。
...
Run: `uv run pytest tests/test_cli.py -v`
Expected: PASS，10 passed
再跑全库 `uv run pytest -q`，预期 **308 passed / 0 skipped**
...
Run: `uv run pytest -q`
Expected: PASS，308 passed, 6 skipped（5 个 generalize + 1 个 llm）

各文件预期数：timecode 14、models 10、config 8、srt_parser 11、clean 24、credits 13、normalize 19、gaps 27（Task 10 实测：原计划写的 21 是按 7 个黄金间隙估的，改成 recall 断言后是 16 单元 + 11 黄金）、density 18、aggregate 15、prompt 12、llm 9、budget 16、validate 20、single 19、table 22、narration 8、pipeline 18、cli 11、e2e 14（+1 skip）、generalize 0（+5 skip）。数字对不上就说明有测试写漏了，回头补。
