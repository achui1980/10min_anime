...
全库预期：`uv run pytest -q` → 341 passed / 2 failed / 1 skipped。剩下 2 个 failed 恰好是
`test_gap_count_is_in_usable_range[akujo_e01]`（34 > 30）与 `[saijo_e05]`（33 > 30），
是待单独处理的间隙数阈值问题，不属于本任务。
